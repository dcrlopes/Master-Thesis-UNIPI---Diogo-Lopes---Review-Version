...
Table~\ref{tab:intro-submarines} gives the size of each national fleet, 
about 150 submarines in total at the end of 2019, together with the nuclear-powered surface ships in service~\cite{wna_ships}.

\begin{table}[!htbp]
  \centering
  \caption{Nuclear-powered vessels in service by country.}
  \label{tab:intro-submarines}
  \small
  \begin{tabular}{@{}lrl@{}}
    \toprule
    Country & Submarines & Surface ships \\
    \midrule
    United States  & 70 & 11 Aircraft Carriers \\
    Russia         & 40 & 6 Icebreakers \\
    China          & 19 & --- \\
    United Kingdom & 10 & --- \\
    France         & 9  & 1 Aircraft Carrier \\
    India          & 3  & --- \\
    \bottomrule
  \end{tabular}
  \vspace{2pt}
  
  {\footnotesize Data from~\cite{wna_ships}. Submarines at the end of 2019,
  carriers in 2017, icebreakers in 2022.}
\end{table}
\FloatBarrier
...
1953~\cite{wna_ships}. The United States programme
...
The plant serves to validate the design and to test the
operating conditions of this nuclear propulsion
system~\cite{ctmsp_labgene_2015}. The land-based prototype reactor, the
...
\begin{enumerate}
  ...
  \item \textbf{The vessel envelope:} A structural dissertation on the
        submarine reactor gives the vessel dimensions
        adopted according to Centro Tecnológico da Marinha em São Paulo (CTMSP). The cylindrical inner radius is
...
The present study is therefore built exclusively from open sources, and the
...
The absence of a public specification also drives the form of the
...
\begin{enumerate}
  \item A multi-year cycle under the LEU limit needs high enrichment,
  ...
  \item The enrichment zoning and the absorber placement, which the long
        cycle motivates, degrade the radial peaking factor
        $F_{\Delta H}$, i.e. the thermal-margin quantity of first interest
        in this work.
\end{enumerate}
...
solving it, and their costs are very different:
...
If an OpenMC evaluation answered every request of the genetic algorithm, every single search would need several thousand runs of fifteen to twenty-five minutes each. For the 4800 evaluations of one search
...
the search at every iteration. Coupling the search directly with OpenMC is
therefore out of reach.

In this work, the genetic algorithm never calls OpenMC. It searches a
Gaussian-process surrogate for a statistical model fitted to the designs
...
\begin{itemize}
  \item A parametric OpenMC model of the core, made by 32 assemblies at 
        pin-level fidelity, used to evaluate the designs.
...
iteration, NSGA-II searches the Gaussian-process surrogate and returns
the designs it predicts to be the best compromises between the two
objectives, no single design being best in both. From that set,
a small batch is selected, six configurations in this work, at the
designs where the surrogate is least certain of its own prediction,
and those six are evaluated in OpenMC (Section~\ref{sec:bg-al}). The
search decides which designs are worth evaluating. The selection of
the batch retains, among them, the six whose predicted objectives are
least supported by the evaluations already performed. A search that
would need thousands of evaluations therefore costs only the OpenMC
runs of the selected designs, at about twenty minutes each.
...
a set of candidate core configurations for LABGENE, from which the
final design can be selected.
The optimisation maximises the cycle length, in effective full-power days 
(EFPD), and minimises the radial peaking factor. Its constraints are 
motivated by licensing practice:
...
\begin{itemize}
  ...
  \item Its enrichment must stay below the LEU limit.
\end{itemize}
...
\begin{itemize}
  ...
  \item Campaign 9 produced a set of five candidate designs on its
        ...
        Every front member clears the soluble boron limit of its own
...
\begin{enumerate}
  ...
  \item The same set of design parameters defines the model at pin,
        assembly and full-core level, so a design evaluated on the single
  ...
  \item Constraints are measured on the model, as the controllability
        constraint is, in place of bounds derived from assumptions, and the
...
\begin{enumerate}
  ...
  \item Thermal-hydraulic feedback is absent. Temperatures are fixed at
        ...
        limit on selected designs, and is never imposed as a
  ...
  \item The controllability constraint tests whether the regulating banks
  ...
  \item The statistical error of the peaking objective was bounded but
        not measured inside the loop. Measured a posteriori on the
...
\begin{itemize}
  ...
  \item Chapter~\ref{ch:methodology} presents the reactor model, the
        ...
        controllability constraint, the soluble boron with the moderator
        temperature coefficient and the boron limit, and the campaign
