...
design is held within the two criteria, and three of them follow
...
design is stored with its inputs and its results, and it is the data set
...
\begin{itemize}
  ...
  \item Campaign 7 narrows the enrichment range to
...
The Campaign 1 bounds constrain each geometric variable on its own and
leave the two uncoupled. From Campaign 2 the design space is the
buildable region and the diagonal inside it is enforced by
$g_\mathrm{geom}$, for the reason Section~\ref{sec:res-c1} reports.
...
\begin{enumerate}
  \item NSGA-II represents the pin count as a continuous variable inside
  ...
  \item The archive stores both numbers, the value the optimiser
...
at 22.446 pins and built with 24. The positions the added rods take are not free. A gadolinia rod
...
\begin{itemize}
  ...
  \item $\delta$ is the lattice-corner pad of the geometry builder,
        \SI{0.02}{cm}, held as a single constant shared by the builder
        and the constraint.
...
rodded state is solved, and no bank worth appears among the active constraints. The
...
five-bank scheme of Figure~\ref{fig:cr-banks} and the constraint below,
which enter the active constraints in Campaign 7 under the one-change rule
...
\begin{enumerate}
  ...
  \item From Campaign 8, with the first two regulating banks inserted,
        which gives $k_\mathrm{RE12}$. That reading is computed and stored, not
...
only the four regulating banks and leaves the shutdown bank
withdrawn.

The banks are those of the five-bank scheme illustrated in
...
the active constraints from Campaign 7, so it governs Campaigns 7, 8 and 9
only. The regulating banks RE1 to RE4
occupy the sixteen central assemblies and carry B$_4$C absorbers. The
shutdown bank occupies the outer sixteen assemblies and stays withdrawn
throughout, because it is reserved for the scram function and a
design that needs it at power is not controllable in normal
operation.
...
\begin{itemize}
  \item $k_\mathrm{RE}$ is the core effective multiplication factor at
        ...
        sixteen control rod assemblies, and the shutdown bank
...
four diagonal assemblies of the middle ring. It is computed and stored from
...
measured on a sample of designs turns that condition into a bound on
the unrodded eigenvalue.
...
\begin{itemize}
  ...
  \item $\Delta\rho_\mathrm{op}$ is the operating margin, taken here as
        a reactivity of \num{0.01}, that is \SI{1000}{pcm}.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{}>{\raggedright\arraybackslash}p{0.16\textwidth}c>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}X@{}}
    ...
    C7 & 1.126 & core, 2D & four regulating banks, eight C6 designs, worths \num{12174} to \SI{14947}{pcm} (Section~\ref{sec:res-c6-kmax}) \\
    C7 measured & 1.113 to 1.285 & core, 2D & implied by each design's own worth, 60 designs \\
    C8 and C9 & 1.166 & core, 2D from 3D & 0.99 plus the smallest 3D eigenvalue difference, 0.14307, times $L_\mathrm{ax}$ \\
    \bottomrule
  \end{tabularx}
\end{table}
...
7. From Campaign 8, the measured constraint is therefore the binding
constraint, and the derived value is a redundant constraint
(Section~\ref{sec:res-c7-screens}).

The bound of Campaigns 8 and 9 was computed from the eigenvalue
difference $k_\mathrm{eff}^\mathrm{core} - k_\mathrm{RE}$ in place of
the reactivity worth. Equation~\eqref{eq:k-max-derived} applied to the
same three-dimensional solves, where the smallest worth is
\SI{13008}{pcm}, gives 1.169 after multiplication by $L_\mathrm{ax}$. The difference changes no feasibility verdict, because
the one design of each campaign whose unrodded eigenvalue falls between
1.166 and 1.169, C8-51 and C9-6, also fails the controllability
constraint.
...
6 the rates are requested at every block, and the stored burnup label agrees
with the physical burnup.
...
The correction was verified by depleting design C5-7 twice with the
same seed over six steps of \SI{4.0}{MWd/kgHM}, once as a single block
and once as three blocks of two steps, with the output kept as
\texttt{chunk\_test.log} in the code repository. The two arms agree to
56 and \SI{79}{pcm} at the two points before the first restart. After
it, the chunked arm reproduces the single arm one step earlier, within
\SI{132}{pcm} at a label of 16 against \SI{12}{MWd/kgHM} and within
\SI{115}{pcm} at 24 against 16, while the point that follows each
restart sits 1106 and \SI{2194}{pcm} above the state it repeats, the
xenon that decays during the step in which nothing burns. The offset is
therefore exactly one step per restart. Its size on the affected
...
\begin{itemize}
  \item $B_\mathrm{label}$ is the cumulative burnup stored by the
...
resumed campaign does not start if its active constraints differ from
the stored ones.
...
the problem. It applies three corrections established by the
...
\begin{enumerate}
  ...
  \item \textbf{Depletion horizon:} the horizon is raised to
        ...
        stored with the same cycle length, a lower bound on the value
...
\begin{enumerate}
  ...
  \item The controllability constraint is applied at the operating maximum
        as well as at the beginning of life.
...
\begin{itemize}
  \item $t_\mathrm{refl}^{\max}$ is the largest reflector thickness that
        ...
        \SI{5.66}{cm} so that the edge of the design space is strictly feasible.
...
within the Monte Carlo noise, with no drift being resolved at two standard
...
composition carries no gadolinia because the criterion is applied at the 
...
beyond the slabs, with the radial reflector spanning the full height. Its
...
grid points are 1.15, 1.29 and \SI{1.43}{cm}, none of them \SI{1.26}{cm}, so
every evaluation would interpolate between two grid points and carry an
interpolation error in a variable that no longer varies. Its seven
reflector grid points are spread over 2.0 to \SI{19.5}{cm}, of which only two
...
\subsection{The enrichment search range}
...
truncated at the horizon by Equation~\eqref{eq:efpd-clip}.
...
The limit is evaluated outside the optimisation loop, stored in the
...
to the highest setting measures the bias. The study is reported in Section~\ref{sec:res-c1c2}, and its outcome is the
...
Both studies rest on seed replicas, and the same replicas fix how two
estimates are compared. Wherever two designs, or one design at two
...
bias, while the core solves inside each evaluation, which evaluates
...
The design of experiments holds six to nine points per design variable,
24 to 42 points in all, and the literature bounds that choice from both
sides. The rule of thumb for the initial design of a Gaussian-process
surrogate is ten points per variable~\cite{whyte_2020}. The allocation
recommended for surrogate-based optimisation places one third of the
evaluation budget in the sampling plan and two thirds in
infill~\cite{forrester_book_2008}, and a benchmark of Bayesian
optimisation finds no drawback in initial designs as small as
$n_x + 4$ points, where $n_x$ is the number of design variables, with
$7.5\,n_x$ as its default~\cite{leriche_picheny_2021}. This work
sets the design of experiments as a compromise between the computation
time it consumes, at ten to twenty-five minutes per transport
evaluation, and the range those recommendations span, at no fewer than
six points per variable.
...
Campaigns 3 and 6 satisfied the rule. Campaigns 8 and 9 were stopped at
...
objective $m$ over that set and each term of the sum is between zero
...
\begin{itemize}
  \item \textbf{Horizon-aware surrogate objective:} the problem is
        ...
        design that has not reached the target is stored with the
        ...
        the evaluator cannot return. The surrogate objective is truncated at the horizon
        ...
        Here $\tilde{f}_1$ and $f_1$ are the truncated and the raw
        ...
        plateau at the horizon non-dominated sorting ranks the designs by
  ...
  \item \textbf{Feasibility margin:} the base criterion of
        ...
        $\kappa = 1.5$, the value stored for the last Campaign 6
        session and used in Campaigns 8 and 9. Candidates with $s_i \le 0$ are
  ...
  \item \textbf{Batch diversity:} the problem is batch collapse. The
        ...
        block of Campaign 6 that introduced the revision, and the last
        Campaign 6 session, from evaluation 85, ran at
        $\delta_\mathrm{min} = 0.14$.

        At that time, 0.14 was not the default of the code, which
        stood at 0.05, so the value had to be passed explicitly at
        launch. The value passed to the two earlier Campaign 6 sessions
        with batch diversity, from evaluations 61 and 67, is not
        stored. Campaign 7 was launched without it and ran its three
        ...
        checkpoint of every campaign stores the value of its last
        session.
...
when the core solve is added, and $t$ is the wall time without it, with 113
...
The surrogate under test is the one in the loop of
...
\begin{enumerate}
  \item \textbf{Membership through the campaign:} the first test
        follows the front as the campaign builds it, with the
        ...
        joined the set is stored, and so is every design that a later
        evaluation dominated and removed from the set. A front whose
        ...
        not, and its composition depends on where the campaign stopped.
        ...
        cent for three iterations. A design can join the front at the
        ...
        the campaign, and Campaign 8 is the case that separates the
        two.
  ...
  \item \textbf{Membership under the transport noise:} the second test
        ...
        driven by such solves~\cite{erdem_2025_mc_uncertainty}. The
        objectives of every feasible design are perturbed by Gaussian
        noise representing the Monte Carlo uncertainty of the transport
        solves, the non-dominated set is recomputed, and the
        probability that each design belongs to it is estimated over
        many replicates. The cycle length noise of a design follows
        from the eigenvalue uncertainty through the mean reactivity
        ...
        \noindent where $\sigma_\mathrm{cycle}$ is the one-sigma noise
        on the cycle length in EFPD, $\sigma_k$ the one-sigma
        uncertainty of one depletion eigenvalue, $k_\mathrm{BOL}$ the
        beginning-of-life multiplication factor of the assembly,
        $k_\mathrm{target}$ the end-of-cycle target of
        Section~\ref{sec:meth-ktarget} and $T$ the archived cycle
        length in EFPD. The factor $\sqrt{2}$ accounts for the
        interpolation of the end-of-cycle crossing between two
        independently noisy eigenvalues.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item The surrogate-assisted search with the campaign settings on the
        same design space, with a design of experiments followed by infill
...
The grid is deliberately wider than the feasible band of the
...
between grid points. The inverted generational distance IGD is zero when
...
fitted on the search archive is finally evaluated at every grid point,
...
\end{table}
  ...
  \item \textbf{Sources of randomness:} three enter a campaign, and they
        ...
        trajectory. Each campaign is documented by its archived
