...
That prototype is LABGENE, the Laborat\'orio de Gera\c{c}\~ao de Energia
Nucleoel\'etrica~\cite{marinha_relatorio_gestao_2025}. It is the
land-based prototype for the reactor of the submarine \'Alvaro
Alberto~\cite{vasconcelos_fapesp_2025} and is located at the Centro
Industrial Nuclear de Aramar (Cina) in Iper\'o, S\~ao
...
Tecnol\'ogico da Marinha em S\~ao Paulo (CTMSP)~\cite{maruyama_2013}.
...
The design variables, together with the constraint on beginning-of-life
$k_{\mathrm{eff}}$ at 1000~ppm soluble boron, express the reduced-boron
and heavy-burnable-poison strategy characteristic of long-life
naval-heritage cores. The design variables are the following:
...
\begin{itemize}
  \item It estimates its own error. The predictive variance shows where
        ...
        improve it most, which Forrester, S\'obester and Keane call a key
  ...
  \item Its cost is affordable for a modest data set. Exact Gaussian
        ...
        S\'obester and Keane places Kriging models within reach for fewer
...
certain monotonicity conditions hold~\cite{mckay_lhs_1979}. Forrester,
S\'obester and Keane describe further plans used in surrogate
modelling~\cite{forrester_book_2008}:

\begin{itemize}
  \item Full factorial designs sample a regular grid of levels in every
        variable. To keep the same sample density, $n$ points in one
        variable become $n^k$ points in $k$ variables, so the cost rises
        quickly with the dimension.

  \item Optimised Latin hypercubes start from a random Latin hypercube,
        whose projections on each axis are evenly spread but whose points
        may still fall along a diagonal. A space-filling measure, such as
        the maximin criterion of Morris and Mitchell, is then optimised
        to spread the points over the whole space.

  \item Latin hypercubes based on orthogonal arrays extend the even
        spread from single variables to groups of variables.

  \item Sobol sequences and other quasi-random sequences fill the space
        well, and the plan for $n-1$ points is contained in the plan for
        $n$ points. They suit studies whose budget of evaluations is not
        fixed in advance.
\end{itemize}
...
\begin{itemize}
  ...
  \item The NEORL framework packages these algorithms for energy
        applications. NEORL, short for NeuroEvolution Optimization with
...
\begin{itemize}
  ...
  \item scikit-learn, a Python module of machine learning
...
\begin{enumerate}
  ...
  \item The cycle length is computed inside the loop by a Monte Carlo
        ...
        maximised it as an objective. Campaign 9 imposed it as a
        constraint at the five-year requirement and minimised the soluble
...
The burnout shapes the reactivity history. In an assembly with gadolinia
...
uranium depletion~\cite{ippolito_1990}. After burnout the unburned poison
and the fissile material it displaced leave a burnup
penalty~\cite{alam_partI_2019}, which for natural gadolinia is small,
