...
\begin{itemize}
  \item The International Atomic Energy Agency (IAEA) reports more than
        ...
        taken from the 2024 update of its Advanced Reactors Information
...
\begin{itemize}
  \item The KLT-40S is deployed in the floating nuclear power plant
        ...
        (UO$_2$) dispersed in a silumin (aluminium-silicon alloy) matrix at 18.6~wt\% uranium-235,
...
the prototype. The
final campaign, constrained by the controllability constraint of
...
The controllability constraint of the final campaign then bounds the
...
On the multi-objective side, NSGA-II~\cite{deb_nsga2_2002} and the Strength Pareto Evolutionary Algorithm (SPEA) are
...
generation. The Vector Evaluated Genetic Algorithm (VEGA), the Multi-Objective
Genetic Algorithm (MOGA), NSGA and the Niched Pareto Genetic Algorithm
(NPGA) are non-elitist algorithms, while NSGA-II, SPEA and the Pareto
Archived Evolution Strategy (PAES) preserve the elite~\cite{deb_2001}.
...
\begin{enumerate}
  \item Random sampling, in which the input vectors are sampled at random
        from their joint distribution.

  \item Stratified sampling, in which the sample space is divided into
        disjoint strata and a random sample is taken from each stratum,
  ...
  \item Latin hypercube sampling, in which the range of each variable is
        ...
        sampled from each interval, and the values of the different
...
\begin{enumerate}
  ...
  \item \emph{Expensive evaluations.} Each candidate is scored by a
        ...
        number of evaluations is still bounded, which is the motivating
  ...
  \item \emph{The fidelity gap.} Cheap low-fidelity models rank designs
...
\begin{itemize}
  \item It is non-linear. The rates that drive the isotopic evolution
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.26\textwidth}%
    ...
    Cheap low-fidelity ranking plus selective high-fidelity confirmation &
...
\begin{enumerate}
  ...
  \item The optimisation framework is refined between campaigns. The
        ...
        of the core and with the requirements taken from the literature
