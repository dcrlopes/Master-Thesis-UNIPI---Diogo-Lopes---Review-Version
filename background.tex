...
equations are stated with the source from which each is derived, and the
...
designs of the Campaign 9 Pareto set, at the beginning of life with all rods
...
Reactivity expresses the deviation from criticality in a form that adds
...
mass loaded at the beginning of life, and burnup is referred to that same
...
\begin{enumerate}
  ...
  \item The discharge-burnup limit of the fuel, which ends irradiation
        ...
        It is the discharge-burnup limit adopted by this work, defined
        in Section~\ref{sec:meth-atf}.
\end{enumerate}
...
Pareto set, at the beginning of life. Each map shows the fission rate of every pin divided by the mean
...
\begin{enumerate}
  \item It is evaluated at the beginning of life on fresh fuel, where
...
the active batches began are identified a posteriori
...
The same distribution governs the comparison of two independent Monte
Carlo estimates: their difference divided by the standard error of
that difference, written $t$ in this work, follows the Student
distribution under the hypothesis that the two estimates share a mean,
and a value of $|t|$ above about two rejects that hypothesis at the
five per cent level.
...
Section~\ref{sec:res-c1c2}, where the re-solution of the finalists at
high fidelity did not preserve their ranking.
...
reference point is the one plotted in that figure. The other two are
...
\begin{enumerate}
  ...
  \item Across campaigns, the values are not comparable whenever the
        ...
        and no such comparison is made in Chapter~\ref{ch:results}.
\end{enumerate}
...
A constrained problem adds difficulty. Most designs the search
...
\begin{itemize}
  ...
  \item $n_g$ is the number of active inequality constraints of the
        ...
        in Equation~\eqref{eq:constraint_form}.
...
Section~\ref{sec:res-c4}, illustrated in Figure~\ref{fig:bg-nsga2-thesis}(b).
...
the enrichment limit. The feasibility of a design, and its total violation
...
else~\cite{forrester_book_2008}. The search in the previous subsection
...
\begin{enumerate}
  ...
  \item Select the infill batch, the designs that are evaluated with
        ...
        \begin{enumerate}[label=\arabic{enumi}.\arabic*]
          ...
          \item Score the candidates on the constraint surrogates.
