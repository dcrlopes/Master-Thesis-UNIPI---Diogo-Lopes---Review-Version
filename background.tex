...
derivations are not repeated and can be found in reactor analysis
textbooks~\cite{duderstadt_hamilton_1976}. The emphasis falls on the
points where theory shapes decisions taken later.
...
Figure~\ref{fig:bg-k} \reviewercomment{shows this hierarchy measured on the thesis model itself}{R3.3 Minor. The three multiplication factors of the ladder are quoted without their Monte Carlo standard deviations and without the transport settings of the solves. Add sigma to each value and state the fidelity, as every eigenvalue in the results chapter does.}. Each geometric step opens an additional leakage path. The
...
\begin{itemize}
  ...
  \item Full 32-assembly core with reflector and vacuum outer boundary,
        ...
        The transport model is two-dimensional, so the axial leakage is
        absent here. It was measured separately on a finite-height model
        at about \SI{3000}{pcm}, an axial factor of \num{1.0289}
        (Section~\ref{sec:meth-lax}), and it enters the end-of-cycle
        target from Campaign 8.
\end{itemize}
...
\begin{figure}[htbp]
  ...
  \caption{Multiplication ladder of the thesis model at reference conditions.}
  \label{fig:bg-k}
\end{figure}
...
maps linearly onto irradiation time~\cite{lee_2020}. Cycle length is
...
\begin{figure}[htbp]
  ...
  \caption{Illustrative reactivity-versus-burnup curves.}
  \label{fig:bg-dep}
\end{figure}
...
tallying them directly with a track-length estimator, whose form is
given in Section~\ref{sec:bg-mc-tally}~\cite{romano_2021_depletion}.
The tallied rates are expressed in reactions per source particle, while
...
the fuel nuclides, as shown in Figure~\ref{fig:bg-xs}.
...
\begin{enumerate}
  \item \reviewercomment{Reactivity authority scales with the total absorber inventory,}{R3.4 Major. This says the hold-down scales with the product of concentration and pin count, but the paragraph above says its duration is set by self-shielding and surface burn-off, and Campaign 6 found forty dilute pins dominating sixteen concentrated ones at equal reactivity. Authority is therefore not a function of the product alone. Restate as: the achievable hold-down is bounded by the inventory, and its time profile is set by the pin count through self-shielding.}
...
\begin{figure}[htbp]
  ...
  \caption{Thermal absorption cross sections at 2200\,m/s.}
  \label{fig:bg-xs}
\end{figure}
...
the spatial source has settled. \reviewercomment{This diagnostic is attached to every evaluation}{R3.7 Minor. Campaigns 1 and 2 ran without the entropy diagnostic, and Chapter 5 reports designs in Campaigns 7 and 8 whose source had not converged when the active batches began. State what the diagnostic triggers when convergence fails, since at present it is recorded and not acted on inside the loop.}
...
simulates, and two estimators are in common use. The collision estimator
scores at every collision inside the region, with the ratio of the
reaction cross section to the total cross section at the collision
point~\cite{lux_koblinger_1991}.
...
Both are unbiased estimators of the same quantity and can be evaluated
...
histories~\cite{openmc_docs}. Its tallies are scored with the
...
track-length estimates~\cite{romano_2021_depletion}.
...
\begin{enumerate}
  \item The standard deviation of a tally contracts with the inverse
        ...
        (Equation~\eqref{eq:mc-var-exp}). Panel~(a) shows
...
\begin{figure}[htbp]
  ...
  \caption[Monte Carlo statistics of the peaking estimator]{Monte Carlo statistics of the peaking estimator. (a)~Contraction of the standard deviation with the number of active histories. (b)~Distribution of the maximum over 264 noisy cells.}
  \label{fig:bg-mc}
\end{figure}
...
the proxy imposes on candidate designs. What matters is therefore how
well the proxy ranks the designs, which a rank correlation measures.
The absolute error of the proxy is secondary.
...
\begin{figure}[!tbp]
  ...
  \includegraphics[width=0.82\linewidth]{images/bg_pareto_hv.pdf}
   \caption{Dominance, the Pareto front and the hypervolume in the objective orientation of this thesis.}
  \label{fig:bg-pareto}
\end{figure}
...
\begin{figure}[!tbp]
  ...
  \caption[Construction of the hypervolume]{Construction of the hypervolume of Equation~\eqref{eq:hv}. (a)~Reference point. (b)~Rectangle of one member. (c)~Union of all rectangles. (d)~Hypervolume.}
...
\reviewercomment{The point is best made by measuring one unchanged front from three different corners.}{R3.5 Minor. Say that the three hypervolume values below are computed on the synthetic five-member front of the figure and not on a campaign front, so that a reader does not look for 2886 in Chapter 5.} The front used below is the five-member front of
...
Reporting cycle length in years instead of days divides every value by
...
This is why the stopping rule of Section~\ref{sec:meth-loop} is
expressed as a relative gain over successive iterations and not as an
absolute threshold. A percentage of a fixed datum survives the
arbitrariness of that datum, while a threshold value does not.
...
than each other in one respect.
...
design, \reviewercomment{by peeling the population like an onion.}{R3.6 Minor. Informal image. The reference theses keep methodological description literal. Write: by removing successive non-dominated fronts.}
...
before that design becomes non-dominated. The ranks obey three rules:
...
\begin{figure}[!tbp]
  ...
  \caption[Dominance and non-dominated sorting]{Dominance and non-dominated sorting. (a)~The four quadrants of one design $X$. (b)~A population sorted into ranks.}
...
In the extreme case of a population with no feasible member at all, the
algorithm would halt for lack of a comparison. NSGA-II instead extends
...
\begin{itemize}
  ...
  \item $g_j(\mathbf{x})$ is the $j$-th constraint of
        ...
        \reviewercomment{feasible when non-positive.}{R3.1 Major. The sum in Equation~\ref{eq:violation} runs to 5, but the constraint set has five members through Campaign 6 and six from Campaign 7 (Table~\ref{tab:campaigns}), and Campaign 8 records a second control reading. Write the upper limit as the number of constraints of the campaign, J, and state J per campaign, otherwise the equation describes Campaigns 2 to 6 only.}
...
\begin{figure}[!tbp]
  ...
  \caption[Constrained domination on illustrative data]{Constrained domination on illustrative data. (a)~Mixed population. (b)~Population with no feasible design.}
...
\reviewercomment{population of 60 evolved for 80 generations, initialised by Latin}{R3.2 Major. Section~\ref{sec:meth-nsga-setting} of the methodology reports that 60 by 80 is the historical setting, finds it not reproducible across seeds, and adopts 300 by 400 for every infill (Table tab:nsga-sens). This chapter states 60 by 80 as the setting of every iteration, here and in the 4800-evaluation arithmetic of Section sec:bg-moo, and the source carried a note that the value was never checked against the campaign checkpoints. State which campaigns ran which setting, read from the checkpoint command lines, and make the two chapters agree.}
hypercube sampling with a fresh random seed. One complete search
...
The design below the boundary in that figure violates a constraint that
the objective axes cannot show. That is why $V$ is computed over all
the constraints of Table~\ref{tab:constraints} and is not read off the
objective plane.
